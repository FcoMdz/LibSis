...
(req, res) => {
    ...
    sql.query(`SELECT * FROM producto`, (sqlErr, sqlRes) => {
        if(sqlErr){
            ...
            sql.query(`INSERT INTO encargo (Abono, clienteId_cte, estatus) VALUES (?, ?, ?)`, [body.abono, body.idCte, body.estatus], (sqlErr, sqlRes) => {
                if(sqlErr){
                    console.log(sqlErr);
                    res.send({ array:null, 
                            success:false, 
...
(req, res) => {
    ...
    sql.query(`SELECT * FROM producto`, (sqlErr, sqlRes) => {
        if(sqlErr){
            ...
            d.setTime(d.getTime() - (/* UTC-6 */ 6) * 60 * 60 * 1000);
            fecha = d.toISOString().split("T")[0];
            sql.query(`INSERT INTO notaapartado (abono, fecha, clienteId_cte, estatus) VALUES (?, ?, ?, ?)`, [body.abono, fecha, body.idCte, body.estatus], (sqlErr, sqlRes) => {
                if(sqlErr){
                    console.log(sqlErr);
                    res.send({ array:null, 
                            success:false, 
